...
Flightplan.prototype.run = function(task, target, options) {
  ...
  new Fiber(function() {
    ...
    if (options.parallel) {
      logger.info('Running tasks simultaneously on all hosts');
      ...
      logger.info('Running tasks synchronously host by host');

      var count = context.hosts.length;
      context.hosts.slice(0).forEach(function (host, index) {
        logger.info(format('Host %s/%s', index + 1, count));
        context.hosts = [host];
        flights.forEach(function (f) {
          flight.run(f.type, f.fn, context);
        });
        ...
        flight.disconnect();
        logger.info(format('Finish tasks for host %s\%s', index + 1, count));
      });
    }
